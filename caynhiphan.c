...
int countOdd(Node* r) {
    ...
    int count = 0;
    if (r->id % 2 != 0) {
        // Kiểm tra nút hiện tại có id là số lẻ không
        count++;
    }

    // Đệ quy đếm số lượng nút có id là số lẻ trong các nút con của r
    count += countOdd(r->leftChild);
    count += countOdd(r->rightChild);
